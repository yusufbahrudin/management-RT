...
JustGage = function (config) {
  ...
  if (obj.config.donut === true) {
    ...
    maxX =
      dx + widgetW - widgetW / 10 - ((widgetW / 6.666666666666667) * obj.config.gaugeWidthScale) / 2
    maxY = labelY
  } else {
    // HALF *******************************
  ...
  if (obj.config.counter === true) {
    ...
    eve.on('raphael.anim.frame.' + obj.level.id, function () {
      ...
      if (obj.config.textRenderer) {
        ...
        obj.txtValue.attr(
          'text',
          humanFriendlyNumber(Math.floor(currentValue), obj.config.humanFriendlyDecimal) +
            obj.config.symbol
        )
      } else if (obj.config.formatNumber) {
