    ...
    if (b != d)
      if (0 > b || b >= l) this.oApi._fnLog(a, 1, "ColReorder 'from' index is out of bounds: " + b)
        ...
        if (a.aoColumns[b].bVisible) {
          ...
          c = 0
          for (h = a.aoData.length; c < h; c++) null !== a.aoData[c].nTr && s(a.aoData[c].nTr, i, m)
        }
        p(a.aoColumns, b, d)
        c = 0
        ...
        f(a.oInstance).trigger('column-reorder.dt', [
          a,
          { from: b, to: d, mapping: n, drop: e, iFrom: b, iTo: d, aiInvertMapping: n }
        ])
      }
  }
  var i = function (a, b) {
    ...
    _fnMouseMove: function (a) {
      if (null === this.dom.drag) {
        if (
          5 >
          Math.pow(
            Math.pow(a.pageX - this.s.mouse.startX, 2) + Math.pow(a.pageY - this.s.mouse.startY, 2),
            0.5
          )
        )
          return
      ...
      for (var b = 0, d = this.s.aoTargets[0].x, e = 0, g = a.length; e < g; e++)
        e != this.s.mouse.fromIndex && b++,
          a[e].bVisible &&
            ...
            ((d += f(a[e].nTh).outerWidth()), this.s.aoTargets.push({ x: d, to: b }))
      0 !== this.s.fixedRight &&
